...
import { FiGrid, FiCalendar, FiClock, FiCheckCircle, FiArrowRight, FiActivity } from "react-icons/fi";
...
export default async function AdminDashboard() {
  ...
  const stats = [
    { label: "Total Services", value: totalServices, icon: FiGrid, color: "#1e3a8a", bg: "rgba(30, 58, 138, 0.1)" },
    { label: "AI Bookings", value: totalBookings, icon: FiCalendar, color: "#8b5cf6", bg: "rgba(139, 92, 246, 0.1)" },
    { label: "Pending", value: pendingBookings, icon: FiClock, color: "#f59e0b", bg: "rgba(245, 158, 11, 0.1)" },
    { label: "Completed", value: completedBookings, icon: FiCheckCircle, color: "#10b981", bg: "rgba(16, 185, 129, 0.1)" },
  ];
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, i) => (
          <FadeIn key={stat.label} delay={i * 0.1}>
            <div className="card p-8 border-none shadow-lg hover:shadow-2xl">
                    ...
                    recentBookings.map((booking) => (
                      <tr key={booking._id} className="hover:bg-slate-50/30 transition-colors">
                        ...
                        <td className="px-10 py-6 text-right">
                          <span className={`px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest ${booking.status === "pending" ? "bg-amber-100 text-amber-700" :
            ...
            <div className="space-y-4 pt-4">
              <Link href="/admin/services/new" className="block">
                <button className="w-full py-5 px-6 bg-blue-900 text-white rounded-2xl font-bold flex items-center justify-between group hover:shadow-2xl hover:shadow-blue-200 transition-all hover:-translate-y-1">
                  ...
                  <FiPlus className="opacity-50 w-8 group-hover:opacity-100 transition-opacity" />
                </button>
              </Link>
